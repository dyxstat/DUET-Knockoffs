...
simulate_one_param <- function(c_val, beta_val, prop.diff_val, nsam_val, fdr_target, seed, ib) {
  ...
  data_x_1 <- as.data.frame(data_x_1)
  data_x_2 <- as.data.frame(data_x_2)
  ...
  y <- rep(c(rep(1, n.sam.grp1), rep(2, n.sam.grp2)), times = n_data)
  ...
  fdr_vec <- c(DUETknockoff=duetknockoff.fdr)
  power_vec <- c(DUETknockoff=duetknockoff.power)

...
## -----------------------------
## 100 times Monte Carlo over grid (Com2seq)
## -----------------------------

c_grid <- c(1e4, 1e5, 1e6)
beta_grid <- c(2, 4, 6, 8, 10, 12)
prop.diff_grid <- c(0.5, 0.75)
...
simulate_one_param <- function(c_val, beta_val, prop.diff_val, nsam_val, fdr_target, seed) {
  ...
  fdr_vec <- Com2seq=otu.new.omni$fdr
  
  power_vec <- Com2seq=otu.new.omni$sen
